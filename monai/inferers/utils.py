...
def sliding_window_classification(
    ...
    # if output_internel_results:
    #     print(f'Output {len(output_rois)} internel results, with shape {output_rois[0].shape}')
    #     for data, ret in zip(slice_batches, output_rois):
    #         x = torch.softmax(ret, dim=1)
    #         x = torch.argmax(x, dim=1)
    #         name = str(list(x.cpu().numpy()))
    #         nib.save( nib.Nifti1Image(data.cpu().numpy().squeeze(), np.eye(4)), f'/homes/clwang/Data/picc/exp/tmp/{name}.nii.gz')
